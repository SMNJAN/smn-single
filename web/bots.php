        ...
        <nav class="navbar navbar-expand-lg navbar-absolute navbar-transparent">
            <div class="container-fluid">
                <div class="navbar-wrapper">
                    ...
                    <img class="navbar-brand" src="assets/img/logo.png" style="width: 13rem">
                </div>
                ...
                <div class="col-md-12">
                    <div class="card ">
                        <div class="card-header">
                            <h4 class="card-title float-left">Botliste</h4>
                            <button type="button" class="btn btn-primary float-right" data-toggle="modal" data-target="#newmb">Musicbot erstellen</button>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="botlist" class="table tablesorter">
                                    ...
                                    if (count($botlist) == 0){ ?>
                                        <tr>
                                            <td colspan="5" class="text-center">Keine Musicbots vorhanden</td>
                                        </tr>
                                    <?php } else {foreach ($botlist as $bot){?>
                                        <tr>
                                            ...
                                            <td><?= $bot['server'] ?></td>
                                            <td>
                                                <a class="btn btn-primary" href="bot.php?id=<?= $bot['id'] ?>">Verwalten</a>
                                                <?php if ($bot['is_online']){ ?>
                                                    <p class="btn btn-danger ml-2 disabled"><i class="fas fa-power-off"></i></p>
                                                <?php } else { ?>
                                                    <p class="btn btn-success ml-2 disabled"><i class="fas fa-power-off"></i></p>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                    <?php } } ?>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Musicbot erstellen</h4>
            ...
            <form id="createBot">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="botname">Interface Name</label>
                                <input type="text" minlength="3" maxlength="30" class="form-control" id="botname" name="name" placeholder="SMNJAN">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="botnickname">Teamspeak Nickname</label>
                                <input type="text" minlength="3" maxlength="30" class="form-control" name="nickname" id="botnickname" required="required" placeholder="TS3AudioBot">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6  pr-md-1">
                            <div class="form-group">
                                <label for="serverip">Teamspeak Server</label>
                                <input type="text" class="form-control" name="server" required="required" id="serverip" placeholder="smnbots.net">
                            </div>
                        </div>
                        <div class="col-md-6  pr-md-1">
                            <div class="form-group">
                                <label for="serverpw">Server Passwort</label>
                                <input type="text" class="form-control" name="server_password" id="serverpw" placeholder="">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="serverSelect" name="serverselect">
                                    <?php foreach (Config::nodes as $key => $node){ ?>
                                        <option value="<?= $key ?>"><?= $node['name'] ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="createBotBTN">Erstellen</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $('#createBot').submit(function (e) {
        ...
        $.ajax({
            ...
            if (data === "success"){
                smnswal({
                    title: "Musicbot erstellt",
                    text: "Dein Musicbot wurde erstellt",
                    type: "success"
                }).then(function() {
                ...
                smnswal({
                    title: "Fehler",
                    text: "Musicbot konnte nicht erstellt werden oder sich verbinden",
                    type: "error"
                })
            }
        });
